...
void buzzer_setup() {
	...
	// There are two timers capable of PWM, 2 and 3. We are using 2 for the
	// modem, so use 3 for the buzzer.
...
extern "C" void __ISR(_TIMER_3_VECTOR, ipl5) T3_IntHandler(void) {
	...
	// This will break other interrupts and millis() (read+clear+write race
	// condition?)
