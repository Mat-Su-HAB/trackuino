...
enum t_sentence_type { SENTENCE_UNK, SENTENCE_GGA, SENTENCE_RMC };
...
static const int NUM_OF_UNK_PARSERS =
	(sizeof(unk_parsers) / sizeof(t_nmea_parser));
static const int NUM_OF_GGA_PARSERS =
	(sizeof(gga_parsers) / sizeof(t_nmea_parser));
static const int NUM_OF_RMC_PARSERS =
	(sizeof(rmc_parsers) / sizeof(t_nmea_parser));
...
void parse_time(const char *token) {
	// Time can have decimals (fractions of a second), but we only take
	// HHMMSS
...
bool gps_decode(char c) {
	...
	case '\n':
		...
		if (num_tokens && our_checksum == their_checksum) {
			...
			Serial.print(millis());
#endif
			// Return a valid position only when we've got two rmc
			// and gga messages with the same timestamp.
			...
			//
			// 1. The timestamps of the two previous GGA/RMC
			// sentences must match.
			//
			// 2. We just processed a known (GGA/RMC) sentence.
			// Suppose the
			//    contrary: after starting up this module, gga_time
			//    and rmc_time are both equal (they're both
			//    initialized to ""), so (1) holds and we wrongly
			//    report a valid position.
			//
			// 3. The GPS has a valid fix. For some reason, the
			// Venus 634FLPX
			//    reports 24 deg N, 121 deg E (the middle of Taiwan)
			//    until a valid fix is acquired:
			//
			//    $GPGGA,120003.000,2400.0000,N,12100.0000,E,0,00,0.0,0.0,M,0.0,M,,0000*69
			//    (OK!) $GPGSA,A,1,,,,,,,,,,,,,0.0,0.0,0.0*30 (OK!)
			//    $GPRMC,120003.000,V,2400.0000,N,12100.0000,E,000.0,000.0,280606,,,N*78
			//    (OK!) $GPVTG,000.0,T,,M,000.0,N,000.0,K,N*02 (OK!)

			if (sentence_type != SENTENCE_UNK && // Known sentence?
				strcmp(gga_time, rmc_time) == 0 && // RMC/GGA
								   // times
								   // match?
				active) { // Valid fix?
	...
	case '*':
		// Handle as ',', but prepares to receive checksum (ie. do not
		// break)
		at_checksum = true;
		our_checksum ^= c;
	...
	case ',':
		...
		switch (sentence_type) {
		case SENTENCE_UNK:
			if (num_tokens < NUM_OF_UNK_PARSERS
				&& unk_parsers[num_tokens])
				unk_parsers[num_tokens](token);
			break;
		case SENTENCE_GGA:
			if (num_tokens < NUM_OF_GGA_PARSERS
				&& gga_parsers[num_tokens])
				gga_parsers[num_tokens](token);
			break;
		case SENTENCE_RMC:
			if (num_tokens < NUM_OF_RMC_PARSERS
	...
	default:
		...
		if (at_checksum) {
			...
			// Regular NMEA data
			if (offset < 15) { // Avoid buffer overrun (tokens can't
					   // be > 15 chars)
				token[offset] = c;
				offset++;
				our_checksum ^= c;
